  ...
  return (
    ...
    >
      <div className="card-body p-8 items-center text-center">
        <div className="w-24 h-24 relative rounded-full overflow-hidden mb-4 border-2 border-gray-50">
          <Image
            src={friend.picture}
            alt={friend.name}
            fill
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-6">
          {friend.tags.map((tag) => (
            <div
              key={tag}
              className="px-2 py-0.5 bg-[#E8F5E9] text-[#244D3F] text-[10px] font-bold rounded uppercase tracking-wider"
            >
              {tag}
            </div>
          ))}
